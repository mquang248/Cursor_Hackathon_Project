...
export function EditPostModal({ isOpen, onClose, post, onPostUpdated }: EditPostModalProps) {
  ...
  useEffect(() => {
    if (post && isOpen) {
      ...
      setTopic((post as unknown as { topic?: string }).topic || '');
      setTimestamp(post.timestamp || '');
      setImagePreview(post.imageUrl || null);
      setImageFile(null); // Don't set existing image as file to upload
      setError('');
    }
  }, [post, isOpen]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!user || !post) {
      setError(t('Không thể cập nhật bài viết', 'Cannot update post'));
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('Cập nhật thất bại', 'Failed to update'));
      }
    } catch (err) {
      ...
      <div className="relative w-full max-w-xl mx-4 bg-black border border-[#2f3336] rounded-2xl shadow-2xl overflow-hidden animate-slide-up">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#2f3336]">
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold text-gradient-vietnam">
              {t('Chỉnh sửa bài viết', 'Edit Post')}
            </span>
          </div>
          <button
